...
class UsersTab(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(filter_frame, text="Логин (частичный):", bg="#f5f6fa").grid(row=0, column=0, padx=6, pady=4, sticky=tk.W)
        self.filter_login = tk.Entry(filter_frame, width=20)
        self.filter_login.grid(row=0, column=1, padx=4, pady=4)

        tk.Label(filter_frame, text="Активен:", bg="#f5f6fa").grid(row=0, column=2, padx=6, pady=4, sticky=tk.W)
        self.filter_active = ttk.Combobox(filter_frame, values=["", "true", "false"], width=8, state="readonly")
    ...
    def _open_create(self):
        ...
        tk.Label(dlg, text="Логин:", bg="#f5f6fa").grid(row=0, column=0, padx=10, pady=6, sticky=tk.W)
        e_login = tk.Entry(dlg, width=28)
        e_login.grid(row=0, column=1, padx=10, pady=6)

        tk.Label(dlg, text="Пароль:", bg="#f5f6fa").grid(row=1, column=0, padx=10, pady=6, sticky=tk.W)
        ...
        def submit():
            ...
            if resp.status_code == 201:
                messagebox.showinfo("Успех", f"Пользователь создан, ID={resp.json()['id']}", parent=dlg)
                dlg.destroy()
                self._load_list()
    ...
    def _open_update(self):
        ...
        tk.Label(dlg, text="Новый пароль:", bg="#f5f6fa").grid(row=2, column=0, padx=10, pady=6, sticky=tk.W)
        e_pass = tk.Entry(dlg, width=28, show="*")
        e_pass.grid(row=2, column=1, padx=10, pady=6)

        tk.Label(dlg, text="(оставьте поля пустыми,\nчтобы не менять)", bg="#f5f6fa",
    ...
    def _delete(self):
        ...
        tk.Label(dlg, text="ID пользователя:", bg="#f5f6fa").grid(row=0, column=0, padx=10, pady=8, sticky=tk.W)
        e_id = tk.Entry(dlg, width=12)
        ...
        def submit():
            ...
            if not uid_val.isdigit():
                messagebox.showwarning("Внимание", "Введите корректный ID", parent=dlg)
                return
            if not messagebox.askyesno("Подтверждение", f"Удалить пользователя ID={uid_val}?", parent=dlg):
                return
        ...
        tk.Button(dlg, text="Удалить", command=submit, bg="#c0392b", fg="white",
        ...
        def submit():
            ...
            if resp.status_code == 200:
                ...
                result_text.insert(tk.END,
                    f"ID:         {u['id']}\n"
                    f"Логин:      {u['login']}\n"
                    f"Активен:    {u['is_active']}\n"
                    f"Создан:     {u['created_at'][:19]}\n"
                    f"Изменён:    {u['updated_at'][:19]}"
                )
            else:
                result_text.insert(tk.END, f"Ошибка {resp.status_code}:\n{resp.text}")
            result_text.config(state=tk.DISABLED)

        tk.Button(dlg, text="Найти", command=submit, bg="#4a6fa5", fg="white",
                  relief=tk.FLAT, padx=12).grid(row=1, column=0, columnspan=2, pady=6)
...
class AuthTab(tk.Frame):
    ...
    def _build(self):
        ...
        tk.Label(login_frame, text="Логин:", bg="#f5f6fa").grid(row=0, column=0, padx=8, pady=6, sticky=tk.W)
        self.e_login = tk.Entry(login_frame, width=28)
        self.e_login.grid(row=0, column=1, padx=8, pady=6)

        tk.Label(login_frame, text="Пароль:", bg="#f5f6fa").grid(row=1, column=0, padx=8, pady=6, sticky=tk.W)
        ...
        tk.Button(reset_frame, text="Применить", command=self._confirm_reset,
                  bg="#4a6fa5", fg="white", relief=tk.FLAT, padx=10).grid(row=8, column=0, columnspan=2, pady=6)
    ...
    def _confirm_reset(self):
        ...
        if not token or not new_pass:
            ...
            return
        resp = api("post", "/auth/reset-password/confirm",
...
class ResetTokensTab(tk.Frame):
    ...
    def _build(self):
        ...
        self.filter_used = ttk.Combobox(filter_frame, values=["", "true", "false"], width=8, state="readonly")
    ...
    def _open_update(self):
        ...
        tk.Label(dlg, text="ID токена:", bg="#f5f6fa").grid(row=0, column=0, padx=10, pady=6, sticky=tk.W)
        e_id = tk.Entry(dlg, width=12)
        e_id.grid(row=0, column=1, padx=10, pady=6)
        if tid:
            e_id.insert(0, str(tid))

        tk.Label(dlg, text="Отметить как использованный:", bg="#f5f6fa").grid(row=1, column=0, padx=10, pady=6, sticky=tk.W)
        used_var = tk.BooleanVar(value=True)
        tk.Checkbutton(dlg, variable=used_var, bg="#f5f6fa").grid(row=1, column=1, padx=10, pady=6, sticky=tk.W)
        ...
        def submit():
            ...
            if not tid_val.isdigit():
                messagebox.showwarning("Внимание", "Введите корректный ID", parent=dlg)
                return
            resp = api("put", f"/reset-tokens/{tid_val}", json={"is_used": used_var.get()})
            if resp is None:
                return
            if resp.status_code == 200:
    ...
    def _delete(self):
        ...
        tk.Label(dlg, text="ID токена:", bg="#f5f6fa").grid(row=0, column=0, padx=10, pady=8, sticky=tk.W)
        e_id = tk.Entry(dlg, width=12)
        e_id.grid(row=0, column=1, padx=10, pady=8)
        if tid:
            e_id.insert(0, str(tid))
        ...
        def submit():
            ...
            if not tid_val.isdigit():
                messagebox.showwarning("Внимание", "Введите корректный ID", parent=dlg)
                return
            if not messagebox.askyesno("Подтверждение", f"Удалить токен ID={tid_val}?", parent=dlg):
                return
    ...
    def _get_by_id(self):
        ...
        def submit():
            ...
            if not tid_val.isdigit():
                messagebox.showwarning("Внимание", "Введите корректный ID", parent=dlg)
                return
            resp = api("get", f"/reset-tokens/{tid_val}")
            if resp is None:
                return
            result_text.config(state=tk.NORMAL)
            result_text.delete("1.0", tk.END)
            if resp.status_code == 200:
                ...
                result_text.insert(tk.END,
                    ...
                    f"user_id:     {t['user_id']}\n"
                    f"token:       {t['token']}\n"
                    f"expires_at:  {t['expires_at'][:19]}\n"
                    f"is_used:     {t['is_used']}\n"
                    f"created_at:  {t['created_at'][:19]}"
                )
            else:
                result_text.insert(tk.END, f"Ошибка {resp.status_code}:\n{resp.text}")
            result_text.config(state=tk.DISABLED)
...
class App:
    ...
    def _build_header(self):
        ...
        tk.Label(header, text="🔐  Auth Service  |  S1",
                 bg="#4a6fa5", fg="white", font=("Arial", 14, "bold")).pack(side=tk.LEFT, padx=16, pady=10)
        tk.Label(header, text=f"API: {API_BASE_URL}",
                 bg="#4a6fa5", fg="#cce0ff", font=("Arial", 9)).pack(side=tk.RIGHT, padx=16)
