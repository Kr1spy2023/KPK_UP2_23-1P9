...
Сервис отвечает за регистрацию пользователей, вход по логину/паролю, выдачу JWT-токенов и сброс пароля. **Не хранит** профильные данные (ФИО, фото, контакты) — они управляются Profile Service.
...
| Параметр  | Пояснение         | Обязательность | Тип    | Ограничение        | Значение по умолчанию |
|-----------|-------------------|----------------|--------|--------------------|-----------------------|
| username  | Имя пользователя  | Да             | string | существует в БД    | —                     |
| password  | Пароль            | Да             | string | не менее 8 символов | —                    |
...
| Параметр | Пояснение          | Обязательность | Тип    | Ограничение          | Значение по умолчанию |
|----------|--------------------|----------------|--------|----------------------|-----------------------|
| token    | Действующий токен  | Да             | string | существует и активен | —                     |
