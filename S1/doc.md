...
erDiagram
    ...
    USER ||--o{ PASSWORD_RESET_TOKEN : "login → user_id"
```

**Список реляционных связей:**
- `USER.login → PASSWORD_RESET_TOKEN.user_id` — у одного пользователя может быть несколько токенов сброса пароля.
...
| Параметр (англ.) | Пояснение                    | Обязательность | Тип      | Ограничение | Значение по умолчанию |
|------------------|------------------------------|----------------|----------|-------------|-----------------------|
| user_id          | Логин пользователя           | Обязательный   | string   | max 150     | —                     |
...
| user_id          | Логин пользователя     | string   |
| token            | Токен                  | string   |
...
| Параметр (англ.) | Пояснение                      | Тип    |
|------------------|--------------------------------|--------|
| user_id          | Фильтр по логину пользователя  | string |
