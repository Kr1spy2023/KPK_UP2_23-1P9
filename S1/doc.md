...
> Новый токен действителен 24 часа. Старый токен аннулируется.
...
> Метод всегда возвращает `success: true` независимо от наличия email в БД — это намеренное решение для безопасности (защита от перебора аккаунтов). Токен сброса действителен 1 час.
...
| search    | Поиск по username (частичное совпадение) | string  |
